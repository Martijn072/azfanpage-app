        ...
        <div className="relative z-10 flex flex-col items-center justify-center flex-1 px-16 gap-12">
          ...
          {lastFixture && (
            <div className="flex flex-col items-center gap-6 w-full">
              <span className="text-white/50 text-base font-body uppercase tracking-wider">Laatste uitslag</span>
              <div className="flex items-center justify-center gap-8 w-full">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <span className="text-white/90 text-base font-headline font-semibold">{lastFixture.teams.home.name}</span>
                </div>
                <div className="flex items-baseline gap-3">
                  <span className="text-white font-mono text-7xl font-bold">{lastFixture.goals.home ?? '-'}</span>
                  <span className="text-white/40 font-mono text-4xl">–</span>
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  <span className="text-white/90 text-base font-headline font-semibold">{lastFixture.teams.away.name}</span>
                </div>
              </div>
              <span className="text-white/50 text-base font-body">
                {lastFixture.league.name} · {lastFixture.league.round}
              </span>
            </div>
          )}

          <div className="w-48 h-px bg-white/15" />

          {nextFixture && (
            <div className="flex flex-col items-center gap-6 w-full">
              <span className="text-white/50 text-base font-body uppercase tracking-wider">Volgende wedstrijd</span>
              <div className="flex items-center justify-center gap-10 w-full">
                <div className="flex flex-col items-center gap-3">
                  <img src={nextFixture.teams.home.logo} alt={nextFixture.teams.home.name} className="h-20 w-20 object-contain" />
                  <span className="text-white/90 text-base font-headline font-semibold">{nextFixture.teams.home.name}</span>
                </div>
                <div className="flex flex-col items-center gap-1">
                  <span className="text-white text-3xl font-headline font-bold">
                    {format(new Date(nextFixture.fixture.date), "HH:mm")}
                  </span>
                  <span className="text-white/70 text-base font-body capitalize">
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  <span className="text-white/90 text-base font-headline font-semibold">{nextFixture.teams.away.name}</span>
                </div>
              </div>
              <span className="text-white/50 text-base font-body">
